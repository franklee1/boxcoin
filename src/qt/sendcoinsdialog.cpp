...
void SendCoinsDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model, SIGNAL(balanceChanged(uint64_t, uint64_t, uint64_t, uint64_t)), this, SLOT(setBalance(uint64_t, uint64_t, uint64_t, uint64_t)));
        connect(model->getOptionsModel(), SIGNAL(displayUnitChanged(int)), this, SLOT(updateDisplayUnit()));

        // Coin Control
        connect(model->getOptionsModel(), SIGNAL(displayUnitChanged(int)), this, SLOT(coinControlUpdateLabels()));
        connect(model->getOptionsModel(), SIGNAL(coinControlFeaturesChanged(bool)), this, SLOT(coinControlFeatureChanged(bool)));
        connect(model->getOptionsModel(), SIGNAL(transactionFeeChanged(uint64_t)), this, SLOT(coinControlUpdateLabels()));
...
void SendCoinsDialog::setBalance(uint64_t balance, uint64_t stake, uint64_t unconfirmedBalance, uint64_t immatureBalance)
{
    Q_UNUSED(stake);
